...
static AVFrame *opaque_setup_frame(SDL_VoutOverlay_Opaque *opaque,
                                   enum AVPixelFormat format, int width,
                                   int height) {
...
static void func_free_l(SDL_VoutOverlay *overlay) {
    ...
    if (opaque->managed_frame) av_frame_free(&opaque->managed_frame);
    ...
    if (opaque->frame_buffer) av_buffer_unref(&opaque->frame_buffer);

    if (opaque->mutex) SDL_DestroyMutex(opaque->mutex);
...
static int func_fill_frame(SDL_VoutOverlay *overlay, const AVFrame *frame) {
    ...
    switch (overlay->format) {
        ...
        default:
            ALOGE(
                "SDL_VoutFFmpeg_ConvertPicture: unexpected overlay format "
                "%s(%d)",
    ...
    // swscale / direct draw
    /*
     ALOGE("ijk_image_convert w=%d, h=%d, df=%d, dd=%d, dl=%d, sf=%d, sd=%d,
     sl=%d", (int)frame->width, (int)frame->height, (int)dst_format,
    ...
    if (use_linked_frame) {
        // do nothing
                                 ...
                                 swscale_dst_pic.data, swscale_dst_pic.linesize,
                                 frame->format, (const uint8_t **)frame->data,
                                 frame->linesize)) {
        opaque->img_convert_ctx = sws_getCachedContext(
            opaque->img_convert_ctx, frame->width, frame->height, frame->format,
            frame->width, frame->height, dst_format, opaque->sws_flags, NULL,
            NULL, NULL);
        ...
        if (!opaque->no_neon_warned) {
            ...
            ALOGE("non-neon image convert %s -> %s",
...
SDL_VoutOverlay *SDL_VoutFFmpeg_CreateOverlay(int width, int height,
                                              int frame_format,
                                              SDL_Vout *display) {
