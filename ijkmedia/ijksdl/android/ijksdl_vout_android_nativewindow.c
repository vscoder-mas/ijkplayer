...
static void SDL_AMediaCodecBufferProxy_reset(
...
static void SDL_AMediaCodecBufferProxy_destroy(
    SDL_AMediaCodecBufferProxy *proxy) {
    if (!proxy) return;
    SDL_AMediaCodecBufferProxy_reset(proxy);
}

static void SDL_AMediaCodecBufferProxy_destroyP(
    SDL_AMediaCodecBufferProxy **proxy) {
    if (!proxy) return;
...
static void SDL_AMediaCodecBufferProxy_invalidate(
    SDL_AMediaCodecBufferProxy *proxy) {
    SDL_AMediaCodecBufferProxy_reset(proxy);
}
...
static SDL_VoutOverlay *func_create_overlay_l(int width, int height,
                                              int frame_format,
                                              SDL_Vout *vout) {
    switch (frame_format) {
...
static void func_free_l(SDL_Vout *vout) {
    ...
    if (opaque) {
        SDL_AMediaCodecBufferProxy **begin =
            (SDL_AMediaCodecBufferProxy **)ISDL_Array__begin(
                &opaque->overlay_manager);
        SDL_AMediaCodecBufferProxy **end =
            (SDL_AMediaCodecBufferProxy **)ISDL_Array__end(
                &opaque->overlay_manager);
        for (; begin < end; ++begin) {
...
static int func_display_overlay_l(SDL_Vout *vout, SDL_VoutOverlay *overlay) {
    ...
    if (overlay->w <= 0 || overlay->h <= 0) {
        ALOGE("func_display_overlay_l: invalid overlay dimensions(%d, %d)",
              overlay->w, overlay->h);
        return -1;
    }
    ...
    switch (overlay->format) {
        ...
        case SDL_FCC__AMC: {
            ...
            return SDL_VoutOverlayAMediaCodec_releaseFrame_l(overlay, NULL,
                                                             true);
        }
        case SDL_FCC_RV24:
        case SDL_FCC_I420:
...
SDL_Vout *SDL_VoutAndroid_CreateForANativeWindow() {
    ...
    if (ISDL_Array__init(&opaque->overlay_pool, 32)) goto fail;

    opaque->egl = IJK_EGL_create();
    if (!opaque->egl) goto fail;
...
static void SDL_VoutAndroid_SetNativeWindow_l(SDL_Vout *vout,
                                              ANativeWindow *native_window) {
...
void SDL_VoutAndroid_SetNativeWindow(SDL_Vout *vout,
                                     ANativeWindow *native_window) {
    SDL_LockMutex(vout->mutex);
...
static void SDL_VoutAndroid_setAMediaCodec_l(SDL_Vout *vout,
                                             SDL_AMediaCodec *acodec) {
    AMCTRACE("%s(%p)\n", __func__, acodec);
    SDL_Vout_Opaque *opaque = vout->opaque;
...
static SDL_AMediaCodecBufferProxy *SDL_VoutAndroid_obtainBufferProxy_l(
    SDL_Vout *vout, int acodec_serial, int buffer_index,
    SDL_AMediaCodecBufferInfo *buffer_info) {
    SDL_Vout_Opaque *opaque = vout->opaque;
    ...
    if (ISDL_Array__size(&opaque->overlay_pool) > 0) {
        ...
        proxy = (SDL_AMediaCodecBufferProxy *)mallocz(
            sizeof(SDL_AMediaCodecBufferProxy));
    ...
    AMCTRACE("%s: [%d] ++++++++ proxy %d: vout: %d idx: %d fake: %s", __func__,
             proxy->buffer_id, proxy->acodec_serial,
...
SDL_AMediaCodecBufferProxy *SDL_VoutAndroid_obtainBufferProxy(
    SDL_Vout *vout, int acodec_serial, int buffer_index,
    SDL_AMediaCodecBufferInfo *buffer_info) {
    SDL_AMediaCodecBufferProxy *proxy = NULL;
    SDL_LockMutex(vout->mutex);
    proxy = SDL_VoutAndroid_obtainBufferProxy_l(vout, acodec_serial,
                                                buffer_index, buffer_info);
    SDL_UnlockMutex(vout->mutex);
    return proxy;
}
...
static int SDL_VoutAndroid_releaseBufferProxy(SDL_Vout *vout,
                                              SDL_AMediaCodecBufferProxy *proxy,
                                              bool render) {
    int ret = 0;
...
int SDL_VoutAndroid_releaseBufferProxyP(SDL_Vout *vout,
                                        SDL_AMediaCodecBufferProxy **proxy,
                                        bool render) {
    int ret = 0;
...
int SDL_VoutAndroid_releaseBufferProxyP_l(SDL_Vout *vout,
                                          SDL_AMediaCodecBufferProxy **proxy,
                                          bool render) {
    int ret = 0;
    if (!proxy) return 0;
