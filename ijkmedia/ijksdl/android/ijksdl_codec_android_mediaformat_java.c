...
jobject SDL_AMediaFormatJava_getObject(JNIEnv *env,
                                       const SDL_AMediaFormat *thiz) {
...
static jobject getAndroidMediaFormat(JNIEnv *env,
                                     const SDL_AMediaFormat *thiz) {
    if (!thiz) return NULL;
...
static sdl_amedia_status_t SDL_AMediaFormatJava_delete(
    SDL_AMediaFormat *aformat) {
    if (!aformat) return SDL_AMEDIA_OK;
    ...
    SDL_AMediaFormat_Opaque *opaque =
        (SDL_AMediaFormat_Opaque *)aformat->opaque;
    if (opaque) {
...
static bool SDL_AMediaFormatJava_getInt32(SDL_AMediaFormat *aformat,
    ...
    jint ret = J4AC_MediaFormat__getInteger__withCString(
        env, android_media_format, name);
...
static void SDL_AMediaFormatJava_setInt32(SDL_AMediaFormat *aformat,
    ...
    J4AC_MediaFormat__setInteger__withCString(env, android_media_format, name,
                                              value);
    if (J4A_ExceptionCheck__catchAll(env)) {
...
static void SDL_AMediaFormatJava_setBuffer(SDL_AMediaFormat *aformat,
    ...
    if (!opaque->android_byte_buffer) {
        opaque->android_byte_buffer =
            J4AC_ByteBuffer__allocateDirect__asGlobalRef__catchAll(env, size);
        if (!opaque->android_byte_buffer) {
            J4A_FUNC_FAIL_TRACE();
            return;
        }
    ...
    ret = J4AC_ByteBuffer__assignData__catchAll(
        env, opaque->android_byte_buffer, data, size);
    if (ret < 0) {
        J4A_FUNC_FAIL_TRACE();
        return;
    }

    J4AC_MediaFormat__setByteBuffer__withCString(
        env, android_media_format, name, opaque->android_byte_buffer);
    if (J4A_ExceptionCheck__catchAll(env)) {
...
static void setup_aformat(SDL_AMediaFormat *aformat,
                          jobject android_media_format) {
...
SDL_AMediaFormat *SDL_AMediaFormatJava_init(JNIEnv *env,
                                            jobject android_format) {
    SDLTRACE("%s", __func__);
    jobject global_android_media_format =
        (*env)->NewGlobalRef(env, android_format);
    if (J4A_ExceptionCheck__catchAll(env) || !global_android_media_format) {
        return NULL;
    }

    SDL_AMediaFormat *aformat =
        SDL_AMediaFormat_CreateInternal(sizeof(SDL_AMediaFormat_Opaque));
    if (!aformat) {
...
SDL_AMediaFormat *SDL_AMediaFormatJava_createVideoFormat(JNIEnv *env,
                                                         const char *mime,
                                                         int width,
                                                         int height) {
    SDLTRACE("%s", __func__);
