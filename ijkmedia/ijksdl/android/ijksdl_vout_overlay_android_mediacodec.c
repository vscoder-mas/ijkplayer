...
static void overlay_unref(SDL_VoutOverlay *overlay) {
    ...
    SDL_VoutAndroid_releaseBufferProxyP(opaque->vout, &opaque->buffer_proxy,
                                        false);
}
...
inline static bool check_object(SDL_VoutOverlay *object,
                                const char *func_name) {
    if (!object || !object->opaque || !object->opaque_class) {
        ALOGE("%s.%s: invalid pipeline\n", object->opaque_class->name,
              func_name);
        return false;
    }
...
static int func_fill_frame(SDL_VoutOverlay *overlay, const AVFrame *frame) {
    ...
    if (opaque->buffer_proxy)
        SDL_VoutAndroid_releaseBufferProxyP(
            opaque->vout, (SDL_AMediaCodecBufferProxy **)&opaque->buffer_proxy,
            false);
...
SDL_VoutOverlay *SDL_VoutAMediaCodec_CreateOverlay(int width, int height,
                                                   SDL_Vout *vout) {
    SDLTRACE(
...
int SDL_VoutOverlayAMediaCodec_releaseFrame_l(SDL_VoutOverlay *overlay,
                                              SDL_AMediaCodec *acodec,
                                              bool render) {
    if (!check_object(overlay, __func__)) return -1;
