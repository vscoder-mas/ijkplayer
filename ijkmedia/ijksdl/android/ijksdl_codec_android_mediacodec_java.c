...
typedef struct SDL_AMediaCodec_Opaque {
    ...
    //java层MediaCodec.BufferInfo引用
    jobject output_buffer_info;
    bool is_input_buffer_valid;
} SDL_AMediaCodec_Opaque;

jobject SDL_AMediaCodecJava_getObject(JNIEnv *env,
                                      const SDL_AMediaCodec *thiz) {
    if (!thiz || !thiz->opaque) return NULL;
...
SDL_AMediaFormat *SDL_AMediaCodecJava_getOutputFormat(SDL_AMediaCodec *thiz) {
    ...
    SDL_AMediaCodec_Opaque *opaque = (SDL_AMediaCodec_Opaque *)thiz->opaque;
    jobject local_android_format = J4AC_MediaCodec__getOutputFormat__catchAll(
    ...
    SDL_AMediaFormat *aformat =
        SDL_AMediaFormatJava_init(env, local_android_format);
...
static sdl_amedia_status_t SDL_AMediaCodecJava_delete(SDL_AMediaCodec *acodec) {
    ALOGI("%s\n", __func__);
    if (!acodec) return SDL_AMEDIA_OK;
    ...
    if (opaque) {
        if (opaque->android_media_codec) {
            J4AC_MediaCodec__release__catchAll(env,
                                               opaque->android_media_codec);
        }
...
static sdl_amedia_status_t SDL_AMediaCodecJava_configure_surface(
    JNIEnv *env, SDL_AMediaCodec *acodec, const SDL_AMediaFormat *aformat,
    jobject android_surface, SDL_AMediaCrypto *crypto, uint32_t flags) {
    SDLTRACE("%s", __func__);
    ...
    ALOGE("configure acodec:%p format:%p: surface:%p", android_media_codec,
          android_media_format, android_surface);
    //videoCodec.configure(videoFormat, surface, null, 0);
    J4AC_MediaCodec__configure(env, android_media_codec, android_media_format,
                               android_surface, crypto, flags);
...
static ssize_t SDL_AMediaCodecJava_writeInputData(SDL_AMediaCodec *acodec,
                                                  size_t idx,
                                                  const uint8_t *data,
                                                  size_t size) {
    AMCTRACE("%s", __func__);
    ...
    //data:d->pkt_temp.data, size:d->pkt_temp.size
...
ssize_t SDL_AMediaCodecJava_dequeueInputBuffer(SDL_AMediaCodec *acodec,
                                               int64_t timeoutUs) {
    ...
    jobject android_media_codec = opaque->android_media_codec;
    jint idx = J4AC_MediaCodec__dequeueInputBuffer(env, android_media_codec,
                                                   (jlong)timeoutUs);
    if (J4A_ExceptionCheck__catchAll(env)) {
...
sdl_amedia_status_t SDL_AMediaCodecJava_queueInputBuffer(
    SDL_AMediaCodec *acodec, size_t idx, off_t offset, size_t size,
    ...
    SDL_AMediaCodec_Opaque *opaque = (SDL_AMediaCodec_Opaque *)acodec->opaque;
    jobject android_media_codec = opaque->android_media_codec;
    J4AC_MediaCodec__queueInputBuffer(env, android_media_codec, (jint)idx,
                                      (jint)offset, (jint)size, (jlong)time,
                                      (jint)flags);
    if (J4A_ExceptionCheck__catchAll(env)) {
        return SDL_AMEDIA_ERROR_UNKNOWN;
    }
...
ssize_t SDL_AMediaCodecJava_dequeueOutputBuffer(SDL_AMediaCodec *acodec,
...
sdl_amedia_status_t SDL_AMediaCodecJava_releaseOutputBuffer(
    SDL_AMediaCodec *acodec, size_t idx, bool render) {
    AMCTRACE("%s", __func__);

    JNIEnv *env = NULL;
    if (JNI_OK != SDL_JNI_SetupThreadEnv(&env)) {
        ALOGE("%s(%d, %s): SetupThreadEnv failed", __func__, (int)idx,
              render ? "true" : "false");
        return SDL_AMEDIA_ERROR_UNKNOWN;
    }
    ...
    J4AC_MediaCodec__releaseOutputBuffer(env, android_media_codec, (jint)idx,
                                         (jboolean)render);
    if (J4A_ExceptionCheck__catchAll(env)) {
...
static SDL_AMediaCodec *SDL_AMediaCodecJava_init(JNIEnv *env,
                                                 jobject android_media_codec) {
    SDLTRACE("%s", __func__);

    jobject global_android_media_codec =
        (*env)->NewGlobalRef(env, android_media_codec);
    if (J4A_ExceptionCheck__catchAll(env) || !global_android_media_codec) {
        return NULL;
    }

    SDL_AMediaCodec *acodec =
        SDL_AMediaCodec_CreateInternal(sizeof(SDL_AMediaCodec_Opaque));
    if (!acodec) {
...
SDL_AMediaCodec *SDL_AMediaCodecJava_createByCodecName(JNIEnv *env,
                                                       const char *codec_name) {
    SDLTRACE("%s", __func__);

    jobject android_media_codec =
        J4AC_MediaCodec__createByCodecName__withCString__catchAll(env,
                                                                  codec_name);
    ...
    SDL_AMediaCodec *acodec =
        SDL_AMediaCodecJava_init(env, android_media_codec);
    acodec->object_serial = SDL_AMediaCodec_create_object_serial();
