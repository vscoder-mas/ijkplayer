...
static int find_android_channel(int sdl_channel) {
    for (int i = 0; i < NELEM(g_audio_channel_map); ++i) {
        AudioChannelMapEntry *entry = &g_audio_channel_map[i];
        if (entry->sdl_channel == sdl_channel) return entry->android_channel;
    }
...
static SDL_AudioFormat find_sdl_format(int android_format) {
    for (int i = 0; i < NELEM(g_audio_format_map); ++i) {
        ...
        if (entry->android_format == android_format) return entry->sdl_format;
    }
...
static int find_android_format(int sdl_format) {
    for (int i = 0; i < NELEM(g_audio_format_map); ++i) {
        AudioFormatMapEntry *entry = &g_audio_format_map[i];
        if (entry->sdl_format == sdl_format) return entry->android_format;
    }
...
static void SDL_Android_AudioTrack_get_default_spec(
    SDL_Android_AudioTrack_Spec *spec) {
    assert(spec);
...
int audiotrack_get_native_output_sample_rate(JNIEnv *env) {
    ...
    if (J4A_ExceptionCheck__catchAll(env) || retval <= 0) return -1;

    return retval;
}

void SDL_Android_AudioTrack_set_volume(JNIEnv *env,
                                       SDL_Android_AudioTrack *atrack,
                                       float left_volume, float right_volume) {
    J4AC_AudioTrack__setStereoVolume__catchAll(env, atrack->thiz, left_volume,
                                               right_volume);
}

SDL_Android_AudioTrack *SDL_Android_AudioTrack_new_from_spec(
    JNIEnv *env, SDL_Android_AudioTrack_Spec *spec) {
    assert(spec);
    ...
    SDL_Android_AudioTrack *atrack =
        (SDL_Android_AudioTrack *)mallocz(sizeof(SDL_Android_AudioTrack));
    if (!atrack) {
    ...
    int min_buffer_size = J4AC_AudioTrack__getMinBufferSize(
        env, atrack->spec.sample_rate_in_hz, atrack->spec.channel_config,
        atrack->spec.audio_format);
    if (J4A_ExceptionCheck__catchAll(env) || min_buffer_size <= 0) {
        ALOGE("%s: J4AC_AudioTrack__getMinBufferSize: return %d:", __func__,
              min_buffer_size);
        free(atrack);
        return NULL;
    }
    ...
    atrack->thiz = J4AC_AudioTrack__AudioTrack__asGlobalRef__catchAll(
        env, atrack->spec.stream_type, atrack->spec.sample_rate_in_hz,
        atrack->spec.channel_config, atrack->spec.audio_format, min_buffer_size,
        atrack->spec.mode);
    if (!atrack->thiz) {
        free(atrack);
        return NULL;
    ...
    ALOGI("%s: init volume as %f/(%f,%f)", __func__, init_volume,
          atrack->min_volume, atrack->max_volume);
    J4AC_AudioTrack__setStereoVolume__catchAll(env, atrack->thiz, init_volume,
                                               init_volume);

    return atrack;
}

SDL_Android_AudioTrack *SDL_Android_AudioTrack_new_from_sdl_spec(
    JNIEnv *env, const SDL_AudioSpec *sdl_spec) {
...
void SDL_Android_AudioTrack_get_target_spec(SDL_Android_AudioTrack *atrack,
...
int SDL_Android_AudioTrack_reserve_byte_buffer(JNIEnv *env,
                                               SDL_Android_AudioTrack *atrack,
                                               int size_in_byte) {
    ...
    atrack->byte_buffer =
        J4A_NewByteArray__asGlobalRef__catchAll(env, capacity);
    if (!atrack->byte_buffer) return -1;
...
int SDL_Android_AudioTrack_getAudioSessionId(JNIEnv *env,
                                             SDL_Android_AudioTrack *atrack) {
    SDLTRACE("%s", __func__);
...
void SDL_Android_AudioTrack_setSpeed(JNIEnv *env,
                                     SDL_Android_AudioTrack *atrack,
                                     float speed) {
    J4AC_AudioTrack__setSpeed(env, atrack->thiz, speed);
    if (J4A_ExceptionCheck__catchAll(env)) return;
