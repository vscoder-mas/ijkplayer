...
static inline SLmillibel android_amplification_to_sles(float volumeLevel) {
    ...
    if (volumeLevel < 0.00000001) return SL_MILLIBEL_MIN;
    ...
    while (!opaque->abort_request) {
        ...
        if (opaque->need_set_volume) {
            ...
            SLmillibel level = android_amplification_to_sles(
                (opaque->left_volume + opaque->right_volume) / 2);
...
static int aout_thread(void *arg) { return aout_thread_n(arg); }

static void aout_opensles_callback(SLAndroidSimpleBufferQueueItf caller,
                                   void *pContext) {
    SDLTRACE("%s\n", __func__);
...
static void aout_close_audio(SDL_Aout *aout) {
    ...
    if (opaque->slPlayItf)
        (*opaque->slPlayItf)
            ->SetPlayState(opaque->slPlayItf, SL_PLAYSTATE_STOPPED);
    if (opaque->slBufferQueueItf)
        (*opaque->slBufferQueueItf)->Clear(opaque->slBufferQueueItf);

    if (opaque->slBufferQueueItf) opaque->slBufferQueueItf = NULL;
    if (opaque->slVolumeItf) opaque->slVolumeItf = NULL;
    if (opaque->slPlayItf) opaque->slPlayItf = NULL;
...
static int aout_open_audio(SDL_Aout *aout, const SDL_AudioSpec *desired,
    ...
    CHECK_COND_ERROR((desired->format == AUDIO_S16SYS), "%s: not AUDIO_S16SYS",
                     __func__);
    CHECK_COND_ERROR((desired->channels == 2 || desired->channels == 1),
                     "%s: not 1,2 channel", __func__);
    CHECK_COND_ERROR((desired->freq >= 8000 && desired->freq <= 48000),
                     "%s: unsupport freq %d Hz", __func__, desired->freq);
    if (SDL_Android_GetApiLevel() < IJK_API_21_LOLLIPOP &&
        native_sample_rate > 0 && desired->freq < native_sample_rate) {
    ...
    CHECK_OPENSL_ERROR(
        ret,
        "%s: slPlayerObject->GetInterface(SL_IID_ANDROIDSIMPLEBUFFERQUEUE) "
        "failed",
        __func__);
    ...
    CHECK_OPENSL_ERROR(ret, "%s: slBufferQueueItf->RegisterCallback() failed",
                       __func__);

    // set the player's state to playing
    // ret = (*opaque->slPlayItf)->SetPlayState(opaque->slPlayItf,
    ...
    for (int i = 0; i < OPENSLES_BUFFERS; ++i) {
        ret = (*opaque->slBufferQueueItf)
                  ->Enqueue(opaque->slBufferQueueItf,
                            opaque->buffer + i * opaque->bytes_per_buffer,
                            opaque->bytes_per_buffer);
        CHECK_OPENSL_ERROR(ret, "%s: slBufferQueueItf->Enqueue(000...) failed",
                           __func__);
    }
    ...
    opaque->audio_tid = SDL_CreateThreadEx(&opaque->_audio_tid, aout_thread,
                                           aout, "ff_aout_opensles");
    CHECK_COND_ERROR(opaque->audio_tid, "%s: failed to SDL_CreateThreadEx",
                     __func__);
...
static double aout_get_latency_seconds(SDL_Aout *aout) {
    ...
    SLresult slRet =
        (*opaque->slBufferQueueItf)->GetState(opaque->slBufferQueueItf, &state);
    if (slRet != SL_RESULT_SUCCESS) {
        ALOGE("%s failed\n", __func__);
...
SDL_Aout *SDL_AoutAndroid_CreateForOpenSLES() {
    ...
    ret = (*slEngine)->CreateOutputMix(slEngine, &slOutputMixObject, 1, ids1,
                                       req1);
    ...
    CHECK_OPENSL_ERROR(ret, "%s: slOutputMixObject->Realize() failed",
                       __func__);
