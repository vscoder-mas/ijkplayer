...
static int convert_sps_pps(const uint8_t *p_buf, size_t i_buf_size,
    ...
    if (p_nal_size) *p_nal_size = (p_buf[4] & 0x03) + 1;
    p_buf += 5;
    i_data_size -= 5;
...
static void convert_h264_to_annexb(uint8_t *p_buf, size_t i_len,
                                   size_t i_nal_size, H264ConvertState *state) {
    if (i_nal_size < 3 || i_nal_size > 4) return;
    ...
    while (i_len > 0) {
        if (state->nal_pos < i_nal_size) {
            ...
            if (state->nal_pos < i_nal_size) return;
            p_buf[i - 1] = 1;
            p_buf += i;
            i_len -= i;
        }
        if (state->nal_len > INT_MAX) return;
