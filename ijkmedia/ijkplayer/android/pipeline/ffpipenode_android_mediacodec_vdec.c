...
static SDL_AMediaCodec *create_codec_l(JNIEnv *env, IJKFF_Pipenode *node) {
    ...
    if (opaque->jsurface == NULL) {
        ...
        if (acodec) {
            ...
            opaque->acodec_name[sizeof(opaque->acodec_name) /
                                    sizeof(*opaque->acodec_name) -
                                1] = 0;
        }
    }
    ...
    if (opaque->frame_rotate_degrees == 90 ||
        opaque->frame_rotate_degrees == 270) {
    ...
    if (opaque->codecpar->extradata && opaque->codecpar->extradata_size > 0) {
        if ((opaque->codecpar->codec_id == AV_CODEC_ID_H264 &&
            ...
            (opaque->codecpar->codec_id == AV_CODEC_ID_HEVC &&
             opaque->codecpar->extradata_size > 3 &&
             (opaque->codecpar->extradata[0] == 1 ||
              opaque->codecpar->extradata[1] == 1))) {
#if AMC_USE_AVBITSTREAM_FILTER
            if (opaque->codecpar->codec_id == AV_CODEC_ID_H264) {
            ...
            opaque->orig_extradata =
                (uint8_t *)av_mallocz(opaque->codecpar->extradata_size +
                                      FF_INPUT_BUFFER_PADDING_SIZE);
            if (!opaque->orig_extradata) {
                goto fail;
            }
            memcpy(opaque->orig_extradata, opaque->codecpar->extradata,
                   opaque->codecpar->extradata_size);
            for (int i = 0; i < opaque->codecpar->extradata_size; i += 4) {
                ALOGE("csd-0[%d]: %02x%02x%02x%02x\n",
                      opaque->codecpar->extradata_size,
                      (int)opaque->codecpar->extradata[i + 0],
            ...
            if (opaque->codecpar->codec_id == AV_CODEC_ID_H264) {
                if (0 != convert_sps_pps(opaque->codecpar->extradata,
                                         opaque->codecpar->extradata_size,
                ...
                if (0 != convert_hevc_nal_units(
                             opaque->codecpar->extradata,
                             opaque->codecpar->extradata_size, convert_buffer,
                             convert_size, &sps_pps_size, &opaque->nal_size)) {
            ...
            size_t esds_dec_dscr_type_length =
                opaque->codecpar->extradata_size + 0x18;
            ...
            SDL_AMediaFormat_setBuffer(opaque->input_aformat, "csd-0",
                                       convert_buffer, esds_size);
            free(convert_buffer);
        } else {
            // Codec specific data
            // SDL_AMediaFormat_setBuffer(opaque->aformat, "csd-0",
...
static int reconfigure_codec_l(JNIEnv *env, IJKFF_Pipenode *node,
    ...
    if (new_surface) {
        ...
        if (J4A_ExceptionCheck__catchAll(env) || !opaque->jsurface) goto fail;
    } else {
    ...
    amc_ret = SDL_AMediaCodec_configure_surface(
        env, opaque->acodec, opaque->input_aformat, opaque->jsurface, NULL, 0);
...
static int configure_codec_l(JNIEnv *env, IJKFF_Pipenode *node,
                             jobject new_surface) {
    IJKFF_Pipenode_Opaque *opaque = node->opaque;
    int ret = 0;
    sdl_amedia_status_t amc_ret = 0;
...
static int amc_fill_frame(IJKFF_Pipenode *node, AVFrame *frame, int *got_frame,
                          int output_buffer_index, int acodec_serial,
                          SDL_AMediaCodecBufferInfo *buffer_info) {
    IJKFF_Pipenode_Opaque *opaque = node->opaque;
    FFPlayer *ffp = opaque->ffp;
    VideoState *is = ffp->is;

    frame->opaque = SDL_VoutAndroid_obtainBufferProxy(
        opaque->weak_vout, acodec_serial, output_buffer_index, buffer_info);
    if (!frame->opaque) goto fail;
...
static int feed_input_buffer2(JNIEnv *env, IJKFF_Pipenode *node, int64_t timeUs,
                              int *enqueue_count) {
    IJKFF_Pipenode_Opaque *opaque = node->opaque;
    FFPlayer *ffp = opaque->ffp;
    ...
    if (!d->packet_pending || d->queue->serial != d->pkt_serial) {
        ...
        do {
            ...
            if (ffp_packet_queue_get_or_buffering(
                    ffp, d->queue, &pkt, &d->pkt_serial, &d->finished) < 0) {
                ret = -1;
                goto fail;
            }
            if (ffp_is_flush_packet(&pkt) || opaque->acodec_flush_request) {
                ...
                if (SDL_AMediaCodec_isStarted(opaque->acodec)) {
                    if (opaque->input_packet_count > 0) {
                        // flush empty queue cause error on OMX.SEC.AVC.Decoder
                        // (Nexus S)
                        SDL_VoutAndroid_invalidateAllBuffers(opaque->weak_vout);
                        SDL_AMediaCodec_flush(opaque->acodec);
                        opaque->input_packet_count = 0;
                    }
                    // If codec is configured in synchronous mode, codec will
        ...
        if (opaque->ffp->mediacodec_handle_resolution_change &&
            ...
            size_data = av_packet_get_side_data(
                avpkt, AV_PKT_DATA_NEW_EXTRADATA, &size_data_size);
            ...
            if (size_data && size_data_size >= 7) {
                ...
                if (!new_avctx) return AVERROR(ENOMEM);

                avcodec_parameters_to_context(new_avctx, opaque->codecpar);
                av_freep(&new_avctx->extradata);
                new_avctx->extradata =
                    av_mallocz(size_data_size + AV_INPUT_BUFFER_PADDING_SIZE);
                ...
                change_ret = avcodec_decode_video2(new_avctx, frame,
                                                   &got_picture, avpkt);
                if (change_ret < 0) {
                    avcodec_free_context(&new_avctx);
                    return change_ret;
                    ...
                    if (opaque->codecpar->width != new_avctx->width &&
                        ...
                        avcodec_parameters_from_context(opaque->codecpar,
                                                        new_avctx);
                        opaque->aformat_need_recreate = true;
                        ffpipeline_set_surface_need_reconfigure_l(pipeline,
                                                                  true);
                    }
                }
        ...
        if (opaque->codecpar->codec_id == AV_CODEC_ID_H264 ||
            ...
            convert_h264_to_annexb(d->pkt_temp.data, d->pkt_temp.size,
                                   opaque->nal_size, &convert_state);
            int64_t time_stamp = d->pkt_temp.pts;
            if (!time_stamp && d->pkt_temp.dts) time_stamp = d->pkt_temp.dts;
            if (time_stamp > 0) {
                time_stamp = av_rescale_q(time_stamp, is->video_st->time_base,
                                          AV_TIME_BASE_Q);
            } else {
                time_stamp = 0;
            }
    ...
    if (d->pkt_temp.data) {
        ...
        if (input_buffer_index < 0) {
            ...
            copy_size = SDL_AMediaCodec_writeInputData(
                opaque->acodec, input_buffer_index, d->pkt_temp.data,
                d->pkt_temp.size);
            if (!copy_size) {
        ...
        // ALOGE("%s: queue %d/%d", __func__, (int)copy_size,
        // (int)input_buffer_size);
        opaque->input_packet_count++;
        if (enqueue_count) ++*enqueue_count;
    }
...
static int feed_input_buffer(JNIEnv *env, IJKFF_Pipenode *node, int64_t timeUs,
                             int *enqueue_count) {
    IJKFF_Pipenode_Opaque *opaque = node->opaque;
    FFPlayer *ffp = opaque->ffp;
    IJKFF_Pipeline *pipeline = opaque->pipeline;
    ...
    //d->packet_pending=1，表示有未发送完的AVPacket，不取新AVPacket
    if (!d->packet_pending || d->queue->serial != d->pkt_serial) {
#if AMC_USE_AVBITSTREAM_FILTER
#else
        H264ConvertState convert_state = {0, 0};
        ...
        if (d->bfsc_ret > 0) {
            if (d->bfsc_data) av_freep(&d->bfsc_data);
            d->bfsc_ret = 0;
        }
        d->bfsc_ret = av_bitstream_filter_filter(
            opaque->bsfc, opaque->avctx, NULL, &d->pkt_temp.data,
            &d->pkt_temp.size, d->pkt.data, d->pkt.size,
        ...
        AMCTRACE("bsfc->filter(%d): %p[%d] -> %p[%d]", d->bfsc_ret, d->pkt.data,
                 (int)d->pkt.size, d->pkt_temp.data, (int)d->pkt_temp.size);
#else
#if 0
        ...
        if (ffpipeline_is_surface_need_reconfigure_l(pipeline)) {
            ...
            if (!opaque->aformat_need_recreate &&
                ...
                while (!q->abort_request &&
                    ...
                    SDL_CondWaitTimeout(
                        opaque->acodec_first_dequeue_output_cond,
    ...
    /*
        3. pkt_temp更新
        每次发送后，根据已发送大小copy_size更新pkt_temp，直到pkt_temp.size小于0，
        置packet_pending为0。在读Packet前会先判断packet_pending是否为1，如果为1，
        则不拉取新的Packet，而是先消耗pkt_temp。这样就达到循环发送Packet的目的了
    */
    if (copy_size < 0) {
        //无需分包
        d->packet_pending = 0;
...
static int enqueue_thread_func(void *arg) {
    ...
    while (!q->abort_request && !opaque->abort) {
        //主要调用MediaCodec.queueInputBuffer给MediaCodec喂原始数据
...
static double pts_from_buffer_info(IJKFF_Pipenode *node,
                                   SDL_AMediaCodecBufferInfo *buffer_info) {
    IJKFF_Pipenode_Opaque *opaque = node->opaque;
    FFPlayer *ffp = opaque->ffp;
    VideoState *is = ffp->is;
    AVRational tb = is->video_st->time_base;
    int64_t amc_pts = av_rescale_q(buffer_info->presentationTimeUs,
                                   AV_TIME_BASE_Q, is->video_st->time_base);
...
static int drain_output_buffer_l(JNIEnv *env, IJKFF_Pipenode *node,
                                 int64_t timeUs, int *dequeue_count,
    ...
    output_buffer_index = SDL_AMediaCodecFake_dequeueOutputBuffer(
        opaque->acodec, &bufferInfo, timeUs);
    if (output_buffer_index == AMEDIACODEC__INFO_OUTPUT_BUFFERS_CHANGED) {
        ...
        if (opaque->output_aformat) {
            ...
            SDL_AMediaFormat_getInt32(opaque->output_aformat, "height",
                                      &height);
            SDL_AMediaFormat_getInt32(opaque->output_aformat, "color-format",
                                      &color_format);

            SDL_AMediaFormat_getInt32(opaque->output_aformat, "stride",
                                      &stride);
            SDL_AMediaFormat_getInt32(opaque->output_aformat, "slice-height",
                                      &slice_height);
            SDL_AMediaFormat_getInt32(opaque->output_aformat, "crop-left",
                                      &crop_left);
            SDL_AMediaFormat_getInt32(opaque->output_aformat, "crop-top",
                                      &crop_top);
            SDL_AMediaFormat_getInt32(opaque->output_aformat, "crop-right",
                                      &crop_right);
            SDL_AMediaFormat_getInt32(opaque->output_aformat, "crop-bottom",
                                      &crop_bottom);
            ...
            ALOGI(
                ...
                "    slice-height: (%d)\n"
                "    crop:         (%d, %d, %d, %d)\n",
                width, height, SDL_AMediaCodec_getColorFormatName(color_format),
                color_format, stride, slice_height, crop_left, crop_top,
        ...
        ffp->stat.vdps = SDL_SpeedSamplerAdd(
            &opaque->sampler, FFP_SHOW_VDPS_MEDIACODEC, "vdps[MediaCodec]");

        if (dequeue_count) ++*dequeue_count;
...
#ifdef FFP_SHOW_AMC_VDPS
        {
            ...
            if (0 == (opaque->benchmark_frame_count % 240)) {
                ...
                double per_frame_ms =
                    ((double)diff) / opaque->benchmark_frame_count;
                double fps =
                    ((double)opaque->benchmark_frame_count) * 1000 / diff;
                ALOGE("%lf fps, %lf ms/frame, %" PRIu64 " frames\n", fps,
                      per_frame_ms, opaque->benchmark_frame_count);
            }
        }
#endif
#ifdef FFP_AMC_DISABLE_OUTPUT
        if (!(bufferInfo.flags & AMEDIACODEC__BUFFER_FLAG_FAKE_FRAME)) {
            SDL_AMediaCodec_releaseOutputBuffer(opaque->acodec,
                                                output_buffer_index, false);
        }
        goto done;
#endif

        if (opaque->n_buf_out) {
            //如果开启了缓冲区，则对缓冲区内的帧进行pts排序后输出。看代码，
            //目前只有codec是OMX.TI.DUCATI1.才启用。也就是默认MediaCodec的输出都是pts排序好的
            AMC_Buf_Out *buf_out;
            ...
            if (opaque->off_buf_out < opaque->n_buf_out) {
                ...
                if (opaque->last_queued_pts != AV_NOPTS_VALUE &&
                    ...
                    // SDL_AMediaCodec_releaseOutputBuffer(opaque->acodec,
                    // output_buffer_index, false); goto done;
                }
                /* already sorted */
                ...
                if (pts < buf_out->pts) {
                    ret = amc_fill_frame(
                        node, frame, got_frame, output_buffer_index,
                    ...
                    for (i = opaque->off_buf_out - 1; i >= 0; i--) {
                        ...
                        if (pts > buf_out->pts) {
                            ret = amc_fill_frame(
                                node, frame, got_frame, buf_out->port,
                            ...
                            buf_out->info = bufferInfo;
                            buf_out->pts =
                                pts_from_buffer_info(node, &bufferInfo);
                            sort_amc_buf_out(opaque->amc_buf_out,
                                             opaque->n_buf_out);
                            break;
                        }
                    }
                    ...
                    if (i < 0) {
                        ...
                        if (!(bufferInfo.flags &
                            ...
                            SDL_AMediaCodec_releaseOutputBuffer(
                                opaque->acodec, output_buffer_index, false);
                            goto done;
                        }
                    }
                }
            ...
            //如果是output_buffer_index >= 0，则调用amc_fill_frame把bufferinfo填充到AVFrame中
            //amc_fill_frame主要是填充Frame的宽、高、pts等信息，把bufferinfo填入到Opaque中;并没有填充真正的图像数据
            //Frame显示的时候再利用这些信息通过MediaCodec的releasseOutputBuffer进行显示
...
static int drain_output_buffer2_l(JNIEnv *env, IJKFF_Pipenode *node,
                                  int64_t timeUs, int *dequeue_count,
                                  AVFrame *frame, int *got_frame) {
    IJKFF_Pipenode_Opaque *opaque = node->opaque;
    FFPlayer *ffp = opaque->ffp;
...
static int drain_output_buffer(JNIEnv *env, IJKFF_Pipenode *node,
                               int64_t timeUs, int *dequeue_count,
                               AVFrame *frame, int *got_frame) {
    IJKFF_Pipenode_Opaque *opaque = node->opaque;
    SDL_LockMutex(opaque->acodec_mutex);
...
static int drain_output_buffer2(JNIEnv *env, IJKFF_Pipenode *node,
                                int64_t timeUs, int *dequeue_count,
    ...
    while (ret) {
        got_frame = 0;
        ret = drain_output_buffer2_l(env, node, timeUs, dequeue_count, frame,
                                     &got_frame);
    ...
    if (got_frame) {
        ...
        if (ffp->framedrop > 0 ||
            (ffp->framedrop &&
             ffp_get_master_sync_type(is) != AV_SYNC_VIDEO_MASTER)) {
            ...
            if (frame->pts != AV_NOPTS_VALUE) {
                ...
                if (!isnan(diff) && fabs(diff) < AV_NOSYNC_THRESHOLD &&
                    ...
                    if (is->continuous_frame_drops_early > ffp->framedrop) {
                        ...
                        ffp->stat.drop_frame_rate =
                            (float)(ffp->stat.drop_frame_count) /
...
static int func_run_sync_loop(IJKFF_Pipenode *node) {
    ...
    while (!q->abort_request) {
        ret = drain_output_buffer2(env, node, AMC_SYNC_OUTPUT_TIMEOUT_US,
                                   &dequeue_count, frame, frame_rate);
        ret = feed_input_buffer2(env, node, AMC_SYNC_INPUT_TIMEOUT_US,
                                 &enqueue_count);
    }
...
static int func_run_sync(IJKFF_Pipenode *node) {
    ...
    //B. 循环拉取解码数据
    while (!q->abort_request) {
        int64_t timeUs = opaque->acodec_first_dequeue_output_request
                             ? 0
                             : AMC_OUTPUT_TIMEOUT_US;
        got_frame = 0;
        //主要调用MediaCodec.dequeueOutputBuffer从MediaCodec拉解码数据
        ret = drain_output_buffer(env, node, timeUs, &dequeue_count, frame,
                                  &got_frame);
                    ...
                    if (!isnan(diff) && fabs(diff) < AV_NOSYNC_THRESHOLD &&
                        ...
                        if (is->continuous_frame_drops_early > ffp->framedrop) {
                            ...
                            if (frame->opaque) {
                                SDL_VoutAndroid_releaseBufferProxyP(
                                    opaque->weak_vout,
                                    (SDL_AMediaCodecBufferProxy **)&frame
                                        ->opaque,
                                    false);
                            }
                            av_frame_unref(frame);
...
int ffpipenode_config_from_android_mediacodec(FFPlayer *ffp,
                                              IJKFF_Pipeline *pipeline,
                                              SDL_Vout *vout,
                                              IJKFF_Pipenode *node) {
    int ret = 0;
    ...
    ret = avcodec_parameters_from_context(opaque->codecpar,
    ...
    switch (opaque->codecpar->codec_id) {
        ...
        case AV_CODEC_ID_MPEG4:
            ...
            opaque->mcc.profile =
                opaque->codecpar->profile >= 0 ? opaque->codecpar->profile : 0;
            opaque->mcc.level =
                opaque->codecpar->level >= 0 ? opaque->codecpar->level : 1;
            break;
    ...
    ffp_set_video_codec_info(ffp, MEDIACODEC_MODULE_NAME,
...
IJKFF_Pipenode *ffpipenode_init_decoder_from_android_mediacodec(
    ...
    if (!opaque->acodec_cond || !opaque->acodec_cond ||
        !opaque->acodec_first_dequeue_output_mutex ||
        !opaque->acodec_first_dequeue_output_cond) {
    ...
    opaque->acodec = SDL_AMediaCodecJava_createByCodecName(
        env, ffp->mediacodec_default_name);
...
IJKFF_Pipenode *ffpipenode_create_video_decoder_from_android_mediacodec(
    FFPlayer *ffp, IJKFF_Pipeline *pipeline, SDL_Vout *vout) {
    ...
    //3. 创建MediaFormat 主要设置mime type, width, height和csd-0
    ret = recreate_format_l(env, node);
    if (ret) {
        ALOGE("amc: recreate_format_l failed\n");
        goto fail;
