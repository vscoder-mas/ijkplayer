...
static int ijkio_manager_save_tree_to_file(void* parm,
                                           int64_t key,
                                           void* elem) {
    IjkCacheTreeInfo* info = elem;
    FILE* fp = parm;
...
void ijkio_manager_destroy(IjkIOManagerContext* h) {
    ...
    if (h->ijkio_app_ctx) {
        if (h->auto_save_map) {
            ...
            if (map_tree_info_fp) {
                ijk_map_traversal_handle(h->ijkio_app_ctx->cache_info_map,
                                         map_tree_info_fp,
                                         ijkio_manager_save_tree_to_file);
                fclose(map_tree_info_fp);
            }
        }

        ijk_map_traversal_handle(h->ijkio_app_ctx->cache_info_map, NULL,
                                 tree_destroy);
...
static int cmp(const void* key, const void* node) {
    return FFDIFFSIGN(*(const int64_t*)key,
                      ((const IjkCacheEntry*)node)->logical_pos);
}

static void ijkio_manager_parse_cache_info(IjkIOApplicationContext* app_ctx,
                                           char* file_path) {
    ...
    while (!feof(fp)) {
        ...
        if (ijk_av_strstart(string_line, "tree_index:", (const char**)ptr)) {
        ...
        } else if (ijk_av_strstart(string_line, "tree_physical_init_pos:",
                                   (const char**)ptr)) {
            str_len = strlen(*ptr);
            for (int i = 0; i < str_len; i++) {
                if ((*ptr)[i] < '0' || (*ptr)[i] > '9') {
        ...
        } else if (ijk_av_strstart(string_line,
                                   "tree_physical_size:", (const char**)ptr)) {
            str_len = strlen(*ptr);
            for (int i = 0; i < str_len; i++) {
                if ((*ptr)[i] < '0' || (*ptr)[i] > '9') {
                    (*ptr)[i] = '\0';
        ...
        } else if (ijk_av_strstart(string_line, "tree-info-flush",
                                   (const char**)ptr)) {
            ...
            tree_file_size = 0;
                                   ...
                                   "entry_logical_pos:", (const char**)ptr)) {
            str_len = strlen(*ptr);
            for (int i = 0; i < str_len; i++) {
                if ((*ptr)[i] < '0' || (*ptr)[i] > '9') {
                    (*ptr)[i] = '\0';
            ...
            entry_logical_pos = strtoll(*ptr, NULL, 10);
                                   ...
                                   "entry_physical_pos:", (const char**)ptr)) {
            str_len = strlen(*ptr);
            for (int i = 0; i < str_len; i++) {
                if ((*ptr)[i] < '0' || (*ptr)[i] > '9') {
                    (*ptr)[i] = '\0';
        ...
        } else if (ijk_av_strstart(string_line, "entry-info-flush",
                                   (const char**)ptr)) {
            if (cur_tree_info) {
                ...
                entry_ret = ijk_av_tree_insert(&cur_tree_info->root, cur_entry,
                                               cmp, &cur_node);
...
void ijkio_manager_will_share_cache_map(IjkIOManagerContext* h) {
    ...
    ijk_map_traversal_handle(h->ijkio_app_ctx->cache_info_map, map_tree_info_fp,
                             ijkio_manager_save_tree_to_file);
    fclose(map_tree_info_fp);
    if (h->ijkio_app_ctx->fd >= 0) {
...
int ijkio_manager_io_open(IjkIOManagerContext* h,
                          const char* url,
                          int flags,
                          IjkAVDictionary** options) {
    int ret = -1;
    ...
    t = ijk_av_dict_get(*options, "cache_file_path", NULL,
                        IJK_AV_DICT_MATCH_CASE);
    if (t) {
        strcpy(h->ijkio_app_ctx->cache_file_path, t->value);
    }

    t = ijk_av_dict_get(*options, "cache_map_path", NULL,
                        IJK_AV_DICT_MATCH_CASE);
    if (t) {
        strcpy(h->cache_map_path, t->value);

        t = ijk_av_dict_get(*options, "auto_save_map", NULL,
                            IJK_AV_DICT_MATCH_CASE);
        if (t) {
        ...
        if (h->ijkio_app_ctx->cache_info_map &&
            !ijk_map_size(h->ijkio_app_ctx->cache_info_map)) {
            t = ijk_av_dict_get(*options, "parse_cache_map", NULL,
                                IJK_AV_DICT_MATCH_CASE);
            if (t) {
                parse_cache_map_file = (int)strtol(t->value, NULL, 10);
                if (parse_cache_map_file) {
                    ijkio_manager_parse_cache_info(h->ijkio_app_ctx,
                                                   h->cache_map_path);
                }
            }
        }
    ...
    if (inner) {
        ...
        if (h->ijk_ctx_map) {
            ...
            ijk_map_put(h->ijk_ctx_map, (int64_t)(intptr_t)h->cur_ffmpeg_ctx,
                        inner);
        }
...
fail:
    if (inner) {
        ...
        if (h->ijk_ctx_map) {
            ijk_map_remove(h->ijk_ctx_map,
                           (int64_t)(intptr_t)h->cur_ffmpeg_ctx);
        }
...
int ijkio_manager_io_read(IjkIOManagerContext* h,
                          unsigned char* buf,
                          int size) {
    int ret = -1;
    if (!h)
    ...
    IjkURLContext* inner =
        ijk_map_get(h->ijk_ctx_map, (int64_t)(intptr_t)h->cur_ffmpeg_ctx);
...
int64_t ijkio_manager_io_seek(IjkIOManagerContext* h,
                              int64_t offset,
                              int whence) {
    int64_t ret = -1;
