...
static void packet_queue_destroy(PacketQueue *q) {
    ...
    SDL_LockMutex(q->mutex);
    while (q->recycle_pkt) {
        MyAVPacketNode *pkt = q->recycle_pkt;
        if (pkt) q->recycle_pkt = pkt->next;
        av_freep(&pkt);
    }
    SDL_UnlockMutex(q->mutex);
...
static int convert_image(FFPlayer *ffp, AVFrame *src_frame,
    ...
    if (is->viddec.avctx) {
        ...
        if (!display_aspect_ratio.num || !display_aspect_ratio.den) {
            ...
            origin_dar =
                (float)display_aspect_ratio.num / display_aspect_ratio.den;
        }
    } else {
        ret = -1;
        return ret;
    ...
    if (!img_info->frame_img_convert_ctx) {
        img_info->frame_img_convert_ctx = sws_getContext(
            width, height, src_frame->format, dst_width, dst_height,
    ...
    ret =
        sws_scale(img_info->frame_img_convert_ctx,
    ...
    ret = avcodec_encode_video2(img_info->frame_img_codec_ctx, &avpkt,
                                dst_frame, &got_packet);
    ...
    for (;;) {
                    ...
                    case AVMEDIA_TYPE_VIDEO:
                        ...
                        if (ret >= 0) {
                            ffp->stat.vdps = SDL_SpeedSamplerAdd(
                                &ffp->vdps_sampler, FFP_SHOW_VDPS_AVCODEC,
                                "vdps[avcodec]");
        ...
        do {
            ...
            if (d->packet_pending) {
                ...
                if (packet_queue_get_or_buffering(
                    ...
                    if (!sp->uploaded) {
                        if (sp->sub.num_rects > 0) {
                            ...
                            if (sp->sub.rects[0]->text) {
                                strncpy(buffered_text, sp->sub.rects[0]->text,
                                        4096);
                            } else if (sp->sub.rects[0]->ass) {
                                parse_ass_subtitle(sp->sub.rects[0]->ass,
                                                   buffered_text);
                            }
...
static void check_external_clock_speed(VideoState *is) {
    ...
    } else if ((is->video_stream_index < 0 ||
...
static void step_to_next_frame_l(FFPlayer *ffp) {
    ...
    if (is->paused) stream_toggle_pause_l(ffp, 0);
}
...
static double compute_target_delay(FFPlayer *ffp, double delay,
    ...
    if (get_master_sync_type(is) != AV_SYNC_VIDEO_MASTER) {
        ...
        /* -- by bbcallen: replace is->max_frame_duration with
         * AV_NOSYNC_THRESHOLD */
...
static void video_refresh(FFPlayer *opaque, double *remaining_time) {
    ...
    retry:
        if (frame_queue_nb_remaining(&is->pictq) == 0) {
            ...
            if (is->step) {
                is->step = 0;
                if (!is->paused) stream_update_pause_l(ffp);
            }
            SDL_UnlockMutex(ffp->is->play_mutex);
        }
    display:
        ...
        if (!isnan(pts)) {
            ...
            if ((pts * 1000 * 1000 < is->seek_pos) ||
                ...
                if (is->drop_vframe_count == 0) {
                    ...
                    av_log(NULL, AV_LOG_INFO,
                           "video accurate_seek start, is->seek_pos=%lld, "
                           "pts=%lf, is->accurate_seek_time = %lld\n",
                ...
                if ((now - is->accurate_seek_start_time) <=
                    ...
                    av_log(NULL, AV_LOG_WARNING,
                           "video accurate_seek is error, "
                           "is->drop_vframe_count=%d, now = %lld, pts = %lf\n",
                           is->drop_vframe_count, now, pts);
                    video_accurate_seek_fail = 1;  // if KEY_FRAME interval too
                                                   // big, disable accurate seek
                }
            } else {
                av_log(NULL, AV_LOG_INFO,
                ...
                if (video_seek_pos == is->seek_pos) {
                    ...
                    if (video_seek_pos == is->seek_pos &&
                        is->audio_accurate_seek_req && !is->abort_request) {
...
static int configure_filtergraph(AVFilterGraph *graph, const char *filtergraph,
    ...
    if (filtergraph) {
        ...
        if ((ret = avfilter_graph_parse_ptr(graph, filtergraph, &inputs,
                                            &outputs, NULL)) < 0)
            goto fail;
    } else {
    ...
    do {
        ...
        if (got_frame) {
            ...
            if (ffp->enable_accurate_seek && is->audio_accurate_seek_req &&
                !is->seek_req) {
                frame_pts = (frame->pts == AV_NOPTS_VALUE)
                                ? NAN
                                : frame->pts * av_q2d(tb);
                now = av_gettime_relative() / 1000;
                if (!isnan(frame_pts)) {
                    ...
                    if ((audio_clock * 1000 * 1000 < is->seek_pos) ||
                        ...
                        if (is->drop_aframe_count == 0) {
                            ...
                            if (is->accurate_seek_start_time <= 0 &&
                                (is->video_stream_index < 0 ||
                                 is->video_accurate_seek_req)) {
                                is->accurate_seek_start_time = now;
                            }
                            SDL_UnlockMutex(is->accurate_seek_mutex);
                            av_log(NULL, AV_LOG_INFO,
                                   "audio accurate_seek start, "
                                   "is->seek_pos=%lld, audio_clock=%lf, "
                                   "is->accurate_seek_start_time = %lld\n",
                        ...
                        if (!is->video_accurate_seek_req &&
                            is->video_stream_index >= 0 &&
                            audio_clock * 1000 * 1000 >
                                is->accurate_seek_vframe_pts) {
                            ...
                            if ((now - is->accurate_seek_start_time) <=
                                ffp->accurate_seek_timeout) {
                                av_frame_unref(frame);
                                continue;  // drop some old frame when do
                                           // accurate seek
                            } else {
                                audio_accurate_seek_fail = 1;
                            }
                        ...
                        if (audio_seek_pos == is->seek_pos) {
                            av_log(
                                NULL, AV_LOG_INFO,
                                "audio accurate_seek is ok, "
                                "is->drop_aframe_count=%d, audio_clock = %lf\n",
                ...
                if (audio_accurate_seek_fail) {
                    av_log(NULL, AV_LOG_INFO,
                           ...
                           "is->drop_aframe_count=%d, now = %lld, audio_clock "
                           "= %lf\n",
            ...
            if (reconfigure) {
                ...
                if ((ret = configure_audio_filters(ffp, ffp->afilters, 1)) <
                    0) {
    ...
    for (;;) {
        ...
        if (ffp->get_frame_mode) {
            ...
            if (dst_pts < 0) {
                ...
                dst_pts += (ffp->get_img_info->end_time -
                            ffp->get_img_info->start_time) /
        ...
        if (last_w != frame->width || last_h != frame->height ||
            ...
            last_serial != is->viddec.pkt_serial || ffp->vf_changed ||
            ...
            av_log(NULL, AV_LOG_DEBUG,
                   ...
                   (const char *)av_x_if_null(
                       av_get_pix_fmt_name(frame->format), "none"),
    ...
    if (is->swr_ctx) {
        ...
        if (ffp->soundtouch_enable && ffp->pf_playback_rate != 1.0f &&
            ...
            int ret_len = ijk_soundtouch_translate(
                ...
                bytes_per_sample, is->audio_tgt.channels,
                af->frame->sample_rate);
            if (ret_len > 0) {
...
static int stream_component_open(FFPlayer *ffp, int stream_index) {
...
#endif
    if (ffp->fast) avctx->flags2 |= AV_CODEC_FLAG2_FAST;
#if FF_API_EMU_EDGE
    ...
    switch (avctx->codec_type) {
...
#if CONFIG_AVFILTER
        {
            ...
            if ((ret = decoder_start(&is->auddec, audio_thread, ffp,
                                     "ff_audio_dec")) < 0)
                goto out;
        ...
        case AVMEDIA_TYPE_VIDEO:
            ...
            if (ffp->max_fps >= 0) {
                ...
                if (is->video_st->r_frame_rate.den &&
                    is->video_st->r_frame_rate.num) {
...
static int read_thread(void *arg) {
    ...
    if (scan_all_pmts_set)
        av_dict_set(&ffp->format_opts, "scan_all_pmts", NULL,
                    AV_DICT_MATCH_CASE);
    ...
    if (ffp->find_stream_info) {
        ...
        do {
            if (av_stristart(is->filename, "data:", NULL) &&
                orig_nb_streams > 0) {
                for (i = 0; i < orig_nb_streams; i++) {
                    if (!ic->streams[i] || !ic->streams[i]->codecpar ||
                        ic->streams[i]->codecpar->profile ==
                            FF_PROFILE_UNKNOWN) {
                        break;
                    }
                }
    ...
    for (i = 0; i < ic->nb_streams; i++) {
        ...
        if (type >= 0 && ffp->wanted_stream_spec[type] && st_index[type] == -1)
            if (avformat_match_stream_specifier(
                    ic, st, ffp->wanted_stream_spec[type]) > 0)
                st_index[type] = i;
    ...
    for (;;) {
        ...
        if (is->seek_req) {
            ...
            if (ffp->auto_resume) {
                is->pause_req = 0;
                if (ffp->packet_buffering) is->buffering_on = 1;
                ffp->auto_resume = 0;
                stream_update_pause_l(ffp);
            }
            if (is->pause_req) step_to_next_frame_l(ffp);
            SDL_UnlockMutex(ffp->is->play_mutex);
...
#endif
             || (stream_has_enough_packets(is->audio_st, is->audio_stream_index,
                                           &is->audioq, MIN_FRAMES) &&
                 stream_has_enough_packets(is->video_st, is->video_stream_index,
                                           &is->videoq, MIN_FRAMES) &&
                 stream_has_enough_packets(is->subtitle_st,
                                           is->subtitle_stream_index,
            ...
            if (ffp->loop != 1 && (!ffp->loop || --ffp->loop)) {
                ...
                if (completed) {
                    ...
                    while (!is->abort_request && !is->seek_req)
                        SDL_CondWaitTimeout(is->continue_read_thread,
                                            wait_mutex, 100);
                    SDL_UnlockMutex(wait_mutex);
                    if (!is->abort_request) continue;
                } else {
                    completed = 1;
                    ffp->auto_resume = 0;
...
static VideoState *stream_open(FFPlayer *ffp, const char *filename,
    ...
    is->video_refresh_tid = SDL_CreateThreadEx(
        &is->_video_refresh_tid, video_refresh_thread, ffp, "ff_vout");
    ...
    if (ffp->async_init_decoder && !ffp->video_disable &&
        ...
        if (ffp->mediacodec_all_videos || ffp->mediacodec_avc ||
            ...
            decoder_init(&is->viddec, NULL, &is->videoq,
                         is->continue_read_thread);
...
fail:
    ...
    if (is->video_refresh_tid) SDL_WaitThread(is->video_refresh_tid, NULL);
    stream_close(ffp);
    return NULL;
}
...
void ffp_global_init() {
    if (g_ffmpeg_global_inited) return;
...
static int app_func_event(AVApplicationContext *h, int message, void *data,
                          size_t size) {
    if (!h || !h->opaque || !data) return 0;

    FFPlayer *ffp = (FFPlayer *)h->opaque;
    if (!ffp->inject_opaque) return 0;
...
static int ijkio_app_func_event(IjkIOApplicationContext *h, int message,
    ...
    FFPlayer *ffp = (FFPlayer *)h->opaque;
    if (!ffp->ijkio_inject_opaque) return 0;
...
int ffp_seek_to_l(FFPlayer *ffp, long msec) {
    ...
    if (start_time > 0 && start_time != AV_NOPTS_VALUE) seek_pos += start_time;
...
long ffp_get_current_position_l(FFPlayer *ffp) {
    ...
    // If using REAL time and not ajusted, then return the real pos as
    // calculated from the stream the use case for this is primarily when using
    // a custom non-seekable data source that starts with a buffer that is NOT
    // the start of the stream.  We want the get_current_position to return the
    // time in the stream, and not the player's internal clock.
    if (ffp->no_time_adjust) {
        return (long)pos;
    }

    if (pos < 0 || pos < start_diff) return 0;
...
void ffp_packet_queue_abort(PacketQueue *q) { return packet_queue_abort(q); }

void ffp_packet_queue_start(PacketQueue *q) { return packet_queue_start(q); }

void ffp_packet_queue_flush(PacketQueue *q) { return packet_queue_flush(q); }
...
void ffp_audio_statistic_l(FFPlayer *ffp) {
    VideoState *is = ffp->is;
    ffp_track_statistic_l(ffp, is->audio_st, &is->audioq,
                          &ffp->stat.audio_cache);
}

void ffp_video_statistic_l(FFPlayer *ffp) {
    VideoState *is = ffp->is;
    ffp_track_statistic_l(ffp, is->video_st, &is->videoq,
                          &ffp->stat.video_cache);
}
...
void ffp_check_buffering_l(FFPlayer *ffp) {
    VideoState *is = ffp->is;
    int hwm_in_ms =
        ffp->dcc.current_high_water_mark_in_ms;  // use fast water mark for
                                                 // first loading
    ...
    if (hwm_in_ms > 0) {
        ...
        if (is->audio_st && audio_time_base_valid) {
            ...
            av_log(
                ...
                is->audioq.size, hwm_in_bytes, is->audioq.nb_packets,
                MIN_FRAMES);
#endif
        }
        ...
        if (is->video_st && video_time_base_valid) {
            ...
            av_log(
                ...
                is->videoq.size, hwm_in_bytes, is->videoq.nb_packets,
                MIN_FRAMES);
#endif
        }
        ...
        if (cached_duration_in_ms >= 0) {
            ...
            ffp_notify_msg3(ffp, FFP_MSG_BUFFERING_TIME_UPDATE,
    ...
    if (hwm_in_bytes > 0) {
        buf_size_percent =
            (int)av_rescale(cached_size, 1005, hwm_in_bytes * 10);
#ifdef FFP_SHOW_DEMUX_CACHE
    ...
    if (buf_time_percent >= 0) {
        ...
        if (buf_time_percent >= 100) need_start_buffering = 1;
        buf_percent = buf_time_percent;
    } else {
        if (buf_size_percent >= 100) need_start_buffering = 1;
    ...
    if (need_start_buffering) {
        ...
        if (is->buffer_indicator_queue &&
            ...
            if ((is->audioq.nb_packets >= MIN_MIN_FRAMES ||
                 is->audio_stream_index < 0 || is->audioq.abort_request) &&
                (is->videoq.nb_packets >= MIN_MIN_FRAMES ||
                 is->video_stream_index < 0 || is->videoq.abort_request)) {
                ffp_toggle_buffering(ffp, 0);
            }
        }
    }
...
int ffp_set_stream_selected(FFPlayer *ffp, int stream, int selected) {
    ...
    if (selected) {
        switch (codecpar->codec_type) {
            case AVMEDIA_TYPE_VIDEO:
                if (stream != is->video_stream_index &&
                    is->video_stream_index >= 0)
                    stream_component_close(ffp, is->video_stream_index);
                break;
            case AVMEDIA_TYPE_AUDIO:
                if (stream != is->audio_stream_index &&
                    is->audio_stream_index >= 0)
                    stream_component_close(ffp, is->audio_stream_index);
                break;
            case AVMEDIA_TYPE_SUBTITLE:
                if (stream != is->subtitle_stream_index &&
                    is->subtitle_stream_index >= 0)
                    stream_component_close(ffp, is->subtitle_stream_index);
                break;
            default:
                av_log(ffp, AV_LOG_ERROR,
...
int64_t ffp_get_property_int64(FFPlayer *ffp, int id, int64_t default_value) {
    switch (id) {
        case FFP_PROP_INT64_SELECTED_VIDEO_STREAM:
            if (!ffp || !ffp->is) return default_value;
            return ffp->is->video_stream_index;
        case FFP_PROP_INT64_SELECTED_AUDIO_STREAM:
            if (!ffp || !ffp->is) return default_value;
            return ffp->is->audio_stream_index;
        case FFP_PROP_INT64_SELECTED_TIMEDTEXT_STREAM:
            if (!ffp || !ffp->is) return default_value;
            return ffp->is->subtitle_stream_index;
        case FFP_PROP_INT64_VIDEO_DECODER:
            if (!ffp) return default_value;
