...
void ijkmeta_reset(IjkMediaMeta *meta) {
    if (meta && meta->dict) av_dict_free(&meta->dict);
}
...
void ijkmeta_append_child_l(IjkMediaMeta *meta, IjkMediaMeta *child) {
    ...
    if (!meta->children) {
        meta->children = (IjkMediaMeta **)calloc(IJK_META_INIT_CAPACITY,
                                                 sizeof(IjkMediaMeta *));
        ...
        IjkMediaMeta **new_children =
            (IjkMediaMeta **)calloc(new_capacity, sizeof(IjkMediaMeta *));
        if (!new_children) return;

        memcpy(new_children, meta->children,
               meta->children_capacity * sizeof(IjkMediaMeta *));
        free(meta->children);
...
void ijkmeta_set_avformat_context_l(IjkMediaMeta *meta, AVFormatContext *ic) {
    ...
    for (int i = 0; i < ic->nb_streams; i++) {
        ...
        if (codecpar->profile != FF_PROFILE_UNKNOWN) {
            ...
            if (codec) {
                ijkmeta_set_int64_l(stream_meta, IJKM_KEY_CODEC_PROFILE_ID,
                                    codecpar->profile);
                const char *profile =
                    av_get_profile_name(codec, codecpar->profile);
                if (profile)
                    ijkmeta_set_string_l(stream_meta, IJKM_KEY_CODEC_PROFILE,
                                         profile);
                if (codec->long_name)
                    ijkmeta_set_string_l(stream_meta, IJKM_KEY_CODEC_LONG_NAME,
                                         codec->long_name);
                ijkmeta_set_int64_l(stream_meta, IJKM_KEY_CODEC_LEVEL,
                                    codecpar->level);
                if (codecpar->format != AV_PIX_FMT_NONE)
                    ijkmeta_set_string_l(stream_meta,
        ...
        switch (codecpar->codec_type) {
            case AVMEDIA_TYPE_VIDEO: {
                ijkmeta_set_string_l(stream_meta, IJKM_KEY_TYPE,
                                     IJKM_VAL_TYPE__VIDEO);

                if (codecpar->width > 0)
                    ijkmeta_set_int64_l(stream_meta, IJKM_KEY_WIDTH,
                                        codecpar->width);
                if (codecpar->height > 0)
                    ijkmeta_set_int64_l(stream_meta, IJKM_KEY_HEIGHT,
                                        codecpar->height);
                ...
                if (st->avg_frame_rate.num > 0 && st->avg_frame_rate.den > 0) {
                    ijkmeta_set_int64_l(stream_meta, IJKM_KEY_FPS_NUM,
                                        st->avg_frame_rate.num);
                    ijkmeta_set_int64_l(stream_meta, IJKM_KEY_FPS_DEN,
                                        st->avg_frame_rate.den);
                }
                if (st->r_frame_rate.num > 0 && st->r_frame_rate.den > 0) {
                    ijkmeta_set_int64_l(stream_meta, IJKM_KEY_TBR_NUM,
                                        st->avg_frame_rate.num);
                    ijkmeta_set_int64_l(stream_meta, IJKM_KEY_TBR_DEN,
                                        st->avg_frame_rate.den);
                }
                break;
            }
            case AVMEDIA_TYPE_AUDIO: {
                ...
                if (codecpar->sample_rate)
                    ijkmeta_set_int64_l(stream_meta, IJKM_KEY_SAMPLE_RATE,
...
const char *ijkmeta_get_string_l(IjkMediaMeta *meta, const char *name) {
    if (!meta || !meta->dict || !name) return NULL;
...
int64_t ijkmeta_get_int64_l(IjkMediaMeta *meta, const char *name,
                            int64_t defaultValue) {
    if (!meta || !meta->dict) return defaultValue;
